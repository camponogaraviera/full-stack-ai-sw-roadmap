...
WebRTC `is a set (suite)` of web APIs and protocols for `real-time peer-to-peer communication` for audio, video, and data. It uses ICE, with STUN and TURN, to establish connectivity between peers. WebRTC does not define a signaling protocol, so applications need a separate signaling mechanism, commonly WebSockets, to exchange SDP offers/answers and ICE candidates.
