...
Alternatives to `Express.js` (Node.js) for building RESTful APIs include `Fastify` (Node.js) and `FastAPI` (Python).
...
[AWS](https://github.com/camponogaraviera/aws) offers `Elastic Beanstalk` and container-based deployment solutions using `ECS/EKS` with `Fargate`.

Example using `AWS Fargate`:

- Use `Docker` to containerize/package your application workload (e.g., RESTful API, WebSocket API, etc.).
- Push the `container image` to a container registry, such as [Amazon Elastic Container Registry (ECR)](https://aws.amazon.com/ecr/).
- Deploy the containerized application on an orchestrator, such as [Amazon Elastic Container Service (ECS)](https://aws.amazon.com/ecs/) or [Amazon Elastic Kubernetes Service (EKS)](https://aws.amazon.com/eks/), using `Fargate` as the compute engine.
...
[2] https://cloud.google.com/discover/what-is-rest-api
