...
Before CORS, AJAX requests could only be made to servers belonging to the same origin (domain) as the webpage/browser making the request. This was known as "same-origin restriction". Ansatz solutions for this problem at the time were Proxy or JSONP (limited to only GET requests). CORS emerged as a new standard.

Suppose that your website, named `my_domain1.com` has an image tag `<img>` or a script tag `<script>` to load content from a website named `my_domain2.com` via AJAX requests.

The browser will do a preflight request, i.e., an OPTIONS request that has the following headers:

- Origin header, which is the domain and scheme of the webpage making the request.
...
While the response on the backend of your server may have the following headers:
