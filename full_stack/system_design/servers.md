...
- You intentionally keep session or request state in server memory rather than in an external store (e.g., Redis or a database), requiring mechanisms such as sticky sessions (session affinity).
