<div align='center'>
    <h1> System Design </h1>
    <h2> The Celebrity (a.k.a. Hot Spot) Problem </h2>
</div>
...
This is a high-throughput problem that happens when there are too many requests (high traffic) to a given shard, causing it to overload and crash. For example, a viral video on YouTube, a trending topic on Twitter (e.g., Super Bowl), or a public figure/celebrity profile on Instagram.
...
- Solution for Super Bowl or profile page: provisioning an individual compute instance (to handle peak traffic) with a Hot Standby (to ensure high availability) and sufficient Read Capacity Units (to avoid Throttling), along with an auto-scaling feature (for automation).
